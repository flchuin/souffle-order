...
import { ShoppingCart, Minus, Plus, CheckCircle2, Loader2, Printer, QrCode, Trash2, Lock, Ticket, User, PlusCircle } from "lucide-react";
...
// Drinks
const DRINKS = [
  { id: "drink-hojicha", name: "Hojicha (Drink)", price: 3.0 },
] as const;

// One universal cheese topping (RM2)
const CHEESE = { id: "addon-cheese", name: "More Cheese", price: 2.0 } as const;
...
export default function App() {
  ...
  const [marketing, setMarketing] = useState(true); // checked by default
  ...
  function addDrink(id: string) {
    ...
    setCart((prev) => {
      const exist = prev.find((p) => p.sku === d.id);
      if (exist) return prev.map((p) => (p.sku === d.id ? { ...p, qty: p.qty + 1 } : p));
      return [...prev, { sku: d.id, flavorId: "", name: d.name, unitPrice: d.price, qty: 1, addons: [] }];
    });
  }

  // ---- Cheese logic ----
  // Add cheese to a Soufflé line (unlimited per line)
  function addCheeseToSouffle(sku: string) {
    setCart(prev => prev.map(p => {
      if (p.sku !== sku) return p;
      const addons = p.addons ? [...p.addons] : [];
      const exist = addons.find(a => a.id === CHEESE.id);
      if (exist) { exist.qty += 1; }
      else { addons.push({ id: CHEESE.id, name: CHEESE.name, price: CHEESE.price, qty: 1 }); }
      return { ...p, addons };
    }));
  }
  // Add cheese to a Drink line (max 1 per drink unit)
  function addCheeseToDrink(sku: string) {
    setCart(prev => prev.map(p => {
      if (p.sku !== sku) return p;
      const addons = p.addons ? [...p.addons] : [];
      const exist = addons.find(a => a.id === CHEESE.id);
      const maxAllowed = p.qty; // one cheese per drink
      if (exist) { if (exist.qty < maxAllowed) exist.qty += 1; }
      else { addons.push({ id: CHEESE.id, name: CHEESE.name, price: CHEESE.price, qty: 1 }); }
      return { ...p, addons };
    }));
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-neutral-200">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold leading-tight">Soufflé JingglyJiang — Scan to Order</h1>
      ...
      <main className="max-w-5xl mx-auto px-4 pb-28 pt-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mt-2">
          {DRINKS.map((d) => (
            <motion.div key={d.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border p-3 text-left">
              <div className="font-medium">{d.name}</div>
              <div className="text-xs text-neutral-500">{fmt(d.price)}</div>
              <div className="flex gap-2 mt-2">
                <button onClick={() => addDrink(d.id)} className="px-3 py-1.5 rounded-xl border text-xs">Add</button>
                <button onClick={() => {
                  // find or create the drink line, then add cheese (max 1 per unit)
                  const exist = cart.find(c => c.sku === d.id);
                  if (!exist) { addDrink(d.id); setTimeout(() => addCheeseToDrink(d.id), 0); }
                  else { addCheeseToDrink(d.id); }
                }} className="px-3 py-1.5 rounded-xl border text-xs flex items-center gap-1"><PlusCircle className="w-3 h-3"/> Cheese + {fmt(CHEESE.price)}</button>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border p-4">
            ...
            <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="e.g., 012-3456789" inputMode="tel" className="mt-1 w-full border rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900" />
                ...
                cart.map((i) => (
                  <div key={i.sku} className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="font-medium truncate">{i.name} × {i.qty}</div>
                      <div className="text-xs text-neutral-500">{fmt(i.unitPrice)} each</div>
                      {/* Addons display */}
                      {i.addons && i.addons.length > 0 && (
                        <div className="mt-1 space-y-0.5">
                          {i.addons.map((a) => (
                            <div key={a.id} className="text-xs text-neutral-600">+ {a.name} × {a.qty} ({fmt(a.price * a.qty)})</div>
                          ))}
                        </div>
                      )}
                      {/* Add Cheese button for Soufflé lines only */}
                    ...
                    <div className="font-medium whitespace-nowrap">{fmt(i.unitPrice * i.qty + (i.addons || []).reduce((a,ad)=>a+ad.price*ad.qty,0))}</div>
                  </div>
                ))
              )}
              {cart.length > 0 && (
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="relative">
        ...
        {count > 0 && (<span className="absolute -top-2 -right-2 bg-neutral-900 text-white text-[10px] rounded-full px-1.5 py-0.5">{count}</span>)}
      </div>
        ...
        <details>
          <summary className="list-none cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <button disabled={items.length === 0 || placing} onClick={onPlace} className="px-4 py-2 rounded-xl bg-neutral-900 text-white text-sm flex items-center gap-2 disabled:opacity-50">
                ...
                {items.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {items.map((i) => (
                      <div key={i.sku} className="text-sm">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium">{fmt(i.unitPrice * i.qty + (i.addons||[]).reduce((a,ad)=>a+ad.price*ad.qty,0))}</span>
                        </div>
                        {i.addons && i.addons.length > 0 && (
                          <div className="pl-4 mt-0.5 space-y-0.5">
                            {i.addons.map(a => (<div key={a.id} className="text-xs text-neutral-600">+ {a.name} × {a.qty} ({fmt(a.price * a.qty)})</div>))}
                          </div>
                        )}
                      </div>
                    ))}
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      ...
                      <span className="text-xs text-neutral-700 font-medium">Please <span className="underline">pay at the counter</span>. Your order will be confirmed only after payment.</span>
                    </div>
                  </div>
                )}
              </div>
    ...
    <div className="min-h-screen bg-neutral-50">
      <div className="max-w-xl mx-auto p-6">
        <div className="bg-white border rounded-2xl p-6 mt-8">
          <div className="flex items-center gap-3"><CheckCircle2 className="w-6 h-6 text-green-600" /><h2 className="text-xl font-semibold">Order Placed!</h2></div>
          ...
          <div className="mt-4 space-y-1 text-sm">
            {order.items.map((i) => (
              <div key={i.sku} className="">
                <div className="flex items-center justify-between"><span className="truncate">{i.name} × {i.qty}</span><span className="font-medium">{fmt(i.unitPrice * i.qty + (i.addons||[]).reduce((a,ad)=>a+ad.price*ad.qty,0))}</span></div>
                {i.addons && i.addons.length > 0 && (
                  <div className="pl-4 mt-0.5 space-y-0.5">
                    {i.addons.map(a => (<div key={a.id} className="text-xs text-neutral-600">+ {a.name} × {a.qty} ({fmt(a.price * a.qty)})</div>))}
                  </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t mt-2"><span>Total</span><span className="font-semibold">{fmt(order.total)}</span></div>
          </div>

          <div className="mt-6 flex flex-col items-center gap-3"><button onClick={() => window.print()} className="px-4 py-2 rounded-xl border text-sm flex items-center gap-2"><Printer className="w-4 h-4" /> Print receipt</button>{isEvent && <div className="text-xs text-neutral-500">Please allow 10–15 minutes during peak times.</div>}</div>
        </div>

        {/* Customer page: QR removed intentionally */}
        <div className="text-center mt-8"><a href={window.location.pathname} className="underline text-sm">Start a new order</a></div>
      </div>
    </div>
  );
}

function StaffBoard({ authed, setAuthed, orders, updateOrder, deleteOrder, wipeAll }: { authed: boolean; setAuthed: (v: boolean) => void; orders: Order[]; updateOrder: (id: string, patch: Partial<Order>) => void; deleteOrder: (id: string) => void; wipeAll: () => void; }) {
          ...
          {filtered.map((o) => (
            <div key={o.id} className="bg-white rounded-2xl border p-4 flex flex-col">
              ...
              <div className="mt-3 text-sm flex-1">
                ...
                <div className="flex items-center justify-between pt-2 border-t mt-2"><span>Total</span><span className="font-semibold">{fmt(o.total)}</span></div>
                {o.note && <div className="mt-2 text-xs bg-neutral-50 border rounded-lg p-2">Note: {o.note}</div>}
                {o.marketingOptIn && <div className="mt-2 text-xs text-neutral-500">Marketing: Opted in</div>}
              </div>

              <div className="mt-4 flex flex-wrap items-center gap-2">
                {o.status === "New" && (<button onClick={() => updateOrder(o.id, { status: "Paid" })} className="px-3 py-1.5 rounded-xl bg-neutral-900 text-white text-xs">Mark Paid</button>)}
                {o.status !== "Preparing" && (<button onClick={() => updateOrder(o.id, { status: "Preparing" })} className="px-3 py-1.5 rounded-xl border text-xs">Preparing</button>)}
                {o.status !== "Ready" && (<button onClick={() => updateOrder(o.id, { status: "Ready" })} className="px-3 py-1.5 rounded-xl border text-xs">Ready</button>)}
                {o.status !== "Done" && (<button onClick={() => updateOrder(o.id, { status: "Done" })} className="px-3 py-1.5 rounded-xl border text-xs">Done</button>)}
                {o.status !== "Cancelled" && (<button onClick={() => updateOrder(o.id, { status: "Cancelled" })} className="px-3 py-1.5 rounded-xl border text-xs">Cancel</button>)}
