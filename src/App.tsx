...
interface Order {
  ...
  createdAt: number;
  expiresAt?: number;          // pay-by deadline
  note?: string;
  pickupName: string;
  phone?: string;
...
export default function App() {
  ...
  const isEvent = url.searchParams.get("event") === "1" || true; // keep event banner by default
    ...
    const tick = setInterval(() => {
      ...
      const next = current.map(o => {
        if (o.status === "New" && o.expiresAt && now > o.expiresAt) {
          mutated = true; return { ...o, status: "Cancelled" };
        }
        return o;
      });
      if (mutated) { saveAllOrders(next); setOrders(next); }
    }, 30000); // 30s
    return () => { unsub(); clearInterval(tick); };
  }, []);

  const subtotal = useMemo(() =>
    cart.reduce((s, i) => s + (i.unitPrice * i.qty) + (i.addons || []).reduce((a, ad) => a + ad.price * ad.qty, 0), 0),
  [cart]);
  ...
  function addFlavor(f: Flavor) {
    setCart((prev) => {
      ...
      return [...prev, { sku, flavorId: f.id, name: `${PRODUCT.name} — ${f.name}`, unitPrice: priceFor(f), qty: 1, addons: [] }];
    });
  }
  ...
  function addCheeseToSouffle(sku: string) {
    setCart(prev => prev.map(p => {
      ...
      if (exist) exist.qty += 1; else addons.push({ ...CHEESE, qty: 1 });
      return { ...p, addons };
    }));
  }
  function addCheeseToDrink(sku: string) {
    setCart(prev => prev.map(p => {
      ...
      const addons = p.addons ? [...p.addons] : [];
      const exist = addons.find(a => a.id === CHEESE.id);
      const maxAllowed = p.qty; // at most 1 cheese per drink unit
      if (exist) { if (exist.qty < maxAllowed) exist.qty += 1; }
      else addons.push({ ...CHEESE, qty: 1 });
      return { ...p, addons };
    }));
  }
  ...
  if (isStaffMode) return (
    <StaffBoard
      ...
      updateOrder={(id, patch) => {
        ...
        saveAllOrders(next); setOrders(next);
      }}
      deleteOrder={(id) => {
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-neutral-200">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-neutral-500">Online Order • Pay at Counter</p>
          </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      ...
                      <span className="text-xs text-neutral-700 font-medium">
                        Please <span className="underline">pay at the counter</span>. Your order will be confirmed only after payment.
                      </span>
                    </div>
                  </div>
                )}
      ...
      <div className="max-w-xl mx-auto p-6">
        <div className="bg-white border rounded-2xl p-6 mt-8">
          ...
          <div className="mt-4 space-y-1 text-sm">
            ...
            <div className="flex items-center justify-between pt-2 border-t mt-2">
              <span>Total</span><span className="font-semibold">{fmt(order.total)}</span>
            </div>
          </div>
  ...
  if (!authed) return (
    <div className="min-h-screen grid place-content-center bg-neutral-50 p-6">
      <div className="bg-white border rounded-2xl p-6 w-[360px]">
        ...
        <button onClick={() => setAuthed(pin === "1234")} className="mt-3 w-full px-4 py-2 rounded-xl bg-neutral-900 text-white text-sm">
          Enter
        </button>
      </div>
    </div>
        ...
        <div className="flex items-center gap-2 overflow-auto pb-2 mt-3">
          {["Active", "All", "New", "Paid", "Preparing", "Ready", "Done", "Cancelled"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-full border text-sm whitespace-nowrap ${filter === f ? "bg-neutral-900 text-white border-neutral-900" : "bg-white"}`}
            >
              {f}
            </button>
          ))}
          ...
          {filtered.map((o) => (
            <div key={o.id} className="bg-white rounded-2xl border p-4 flex flex-col">
              ...
              <div className="mt-3 text-sm flex-1">
                ...
                <div className="flex items-center justify-between pt-2 border-t mt-2">
                  <span>Total</span><span className="font-semibold">{fmt(o.total)}</span>
                </div>
