...
// Firestore wiring (make sure src/firebase.ts exports these)
import {
  watchOrders, createOrder, updateOrderDoc, deleteOrderDoc, ensureStaffSignedIn
} from "./firebase";
 ...
 * - Staff view: add `?mode=staff` to URL (PIN 1234 in demo).
 */
...
interface Order {
  id: string;                       // human-friendly queue number (e.g., Q-001)
  items: OrderItem[];
  total: number;
  ...
  _docId?: string;                  // Firestore doc id (for updates/deletes)
}

// ---- Queue number (local only) ----
...
export default function App() {
  ...
  // Subscribe to Firestore orders (live)
  useEffect(() => {
    const unsub = watchOrders((rows: any[]) => {
      const typed: Order[] = rows.map((r: any) => ({
        id: r.queue ?? r.id ?? "",  // prefer 'queue' field for display
        items: r.items ?? [],
        total: r.total ?? 0,
        status: r.status ?? "New",
        ...
        _docId: r._docId,           // from firebase.ts mapper
      }));
      setOrders(typed);
    });
    return () => unsub();
  ...
  // Auto-cancel overdue "New" orders by updating Firestore
  useEffect(() => {
    const t = setInterval(async () => {
      const now = Date.now();
      const overdue = orders.filter(
        (o) => o.status === "New" && o.expiresAt && now > o.expiresAt
      );
      for (const o of overdue) {
        if (o._docId) {
          try { await updateOrderDoc(o._docId, { status: "Cancelled" }); } catch {}
        }
      }
    }, 30000);
  ...
  async function submitOrder() {
    ...
    setPlacing(true);
    try {
      const queue = nextQueueNumber(); // human queue number for display
      const payload = {
        queue, // stored in Firestore
        items: cart.map((c) => ({
          sku: c.sku, name: c.name, unitPrice: c.unitPrice, qty: c.qty, addons: c.addons,
        })),
        total: +subtotal.toFixed(2),
        status: "New",
        createdAt: null, // serverTimestamp() is set in createOrder()
      ...
      // Local immediate view (createdAt will be accurate after snapshot; use Date.now() for now)
      const order: Order = {
        id: queue,
        items: payload.items,
        total: payload.total,
  ...
  if (isStaffMode) return (
    <StaffBoard
      ...
      setAuthed={(v) => {
        setStaffAuthed(v);
        if (v) ensureStaffSignedIn(); // enable anon auth for staff updates
      }}
      orders={orders}
      updateOrder={async (id, patch) => {
        const docId = orders.find(o => o.id === id)?._docId;
        if (!docId) return;
        await updateOrderDoc(docId, patch);
      }}
      deleteOrder={async (id) => {
        const docId = orders.find(o => o.id === id)?._docId;
        if (!docId) return;
        await deleteOrderDoc(docId);
      }}
      wipeAll={async () => {
        if (!confirm("Clear ALL orders?")) return;
        // Demo-only: delete each visible order
        for (const o of orders) {
          if (o._docId) { try { await deleteOrderDoc(o._docId); } catch {} }
        }
      }}
    />
  );
...
function CartButton({ count, subtotal, onClear }: { count: number; subtotal: number; onClear: () => void }) {
  return (
    <div className="flex items-center gap-3">
      {count > 0 && (<button onClick={onClear} className="text-xs text-neutral-600 hover:text-neutral-900 underline">Clear</button>)}
        ...
        <div className="bg-white border rounded-2xl p-6 mt-8">
          ...
          <p className="text-sm text-neutral-600 mt-2">
            ...
            {order.expiresAt && (
              <> Pay by <span className="font-medium">{fmtTime(order.expiresAt)}</span>.{" "}
                {remainingMs > 0 ? (
                  <span className="text-neutral-800">Time left: {remMin}:{String(remSec).padStart(2,"0")}</span>
                ) : (
          ...
          <div className="mt-4 space-y-1 text-sm">
            <div className="text-xs text-neutral-500">
              Placed: {fmtTime(order.createdAt)}{order.expiresAt && <> • Pay by: {fmtTime(order.expiresAt)}</>}
            </div>
            {order.items.map((i) => (
              <div key={i.sku}>
                <div className="flex items-center justify-between">
...
function StaffBoard({
  authed, setAuthed, orders, updateOrder, deleteOrder, wipeAll
}: {
  authed: boolean;
  setAuthed: (v: boolean) => void;
  orders: Order[];
  updateOrder: (id: string, patch: Partial<Order>) => void | Promise<void>;
  deleteOrder: (id: string) => void | Promise<void>;
  wipeAll: () => void | Promise<void>;
}) {
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
          {filtered.map((o) => (
            <div key={o._docId ?? o.id} className="bg-white rounded-2xl border p-4 flex flex-col">
              <div className="flex items-start justify-between gap-3">
                <div>
              ...
              <div className="mt-3 text-sm flex-1">
                <div className="text-[11px] text-neutral-500">
                  Placed: {fmtTime(o.createdAt)}{o.expiresAt && <> • Pay by: {fmtTime(o.expiresAt)}</>}
                </div>
                {o.items.map((i) => (
                  <div key={i.sku}>
                    <div className="flex items-center justify-between">
        ...
        <div className="text-right mt-6">
          <button onClick={() => wipeAll()} className="px-3 py-1.5 rounded-xl border text-xs">Clear All Orders (demo)</button>
        </div>
      </main>
    </div>
  );
